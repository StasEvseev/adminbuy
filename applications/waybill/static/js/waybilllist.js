var app = angular.module('myApp', ['ngRoute', 'ng-breadcrumbs', 'ngResource', 'ui.bootstrap', 'ngSanitize', 'ui.select',
    'AuthModule', 'ngTable', 'RestApp', 'TableHelper', 'FilterApp', 'Params', 'ModalApp', 'ElemsModule']);

app.constant("rootUrl", "/admin/waybillcustomview");
...
//            debugger
    ...
    .controller("FromMailCtrl", function($scope, breadcrumbs, DateWidgetScope, ReceiverItems,
                                         PointSaleItems, InvoiceItems, wrapItemsToChangeCount, stateService, InvoiceItemHelper) {
        $scope.breadcrumbs = breadcrumbs;
        $scope.model = {};
        $scope.model.date = {};
        $scope.model.typeT = 1;
...
//        };-->
        ...
        $scope.next = function() {
            stateService.setStateAttr(stateService.MAIL, $scope.selected_mail);
            stateService.setStateAttr(stateService.TYPEINVOICE, $scope.typeInvoice);
            stateService.setStateAttr(stateService.POINTSALEFROM, undefined);
            stateService.setStateAttr(stateService.ACCEPTANCE, undefined);
            $location.path("/admin/waybillcustomview/select_mail/blank");
        };
    ...
    .config(function($routeProvider, $locationProvider, rootUrl) {
        var staticPath = "/static/waybill/template/";
      $routeProvider
          .when(rootUrl, {
              templateUrl: staticPath + 'waybilllist.html',
              controller: 'WayBillsController',
              label: 'Список накладных'
          })
          .when(rootUrl + '/:id/edit', {
              templateUrl: staticPath + 'waybilledit.html',
              controller: 'WayBillEditController',
              label: 'Редактирование'
          })

          .when(rootUrl + '/select_acceptance', {
              templateUrl: staticPath + 'waybillselectacceptance.html',
              controller: 'WayBillSelectAcceptanceCtrl',
              label: 'Выбор приемки'
          })

          .when(rootUrl + '/select_mail', {
              templateUrl: staticPath + 'from_mail.html',
              controller: 'WayBillMailCtrl',
              label: "Выбор письма"
          })

          .when(rootUrl + '/select_point', {
              templateUrl: staticPath + 'waybillselectpoint.html',
              controller: 'WayBillSelectPointCtrl',
              label: 'Выбор точки-отправителя'
          })

          .when(rootUrl + '/select_acceptance/new_acceptance', {
              templateUrl: staticPath + 'waybillfromacceptance.html',
              controller: 'WayBillFromAcceptanceCtrl',
              label: 'Создание новой накладной'
          })

          .when(rootUrl + '/select_mail/blank', {
              templateUrl: staticPath + 'from_mail_acceptance.html',
              controller: 'FromMailCtrl',
              label: 'Создание новой накладной'
          })

          .when(rootUrl + '/select_mail/new_acceptance', {
              templateUrl: staticPath + 'waybillfromacceptance.html',
              controller: 'WayBillFromAcceptanceCtrl',
              label: "Создание новой накладной"
          })

          .when(rootUrl + '/select_acceptance/new_acceptance/blank', {
              templateUrl: staticPath + 'waybillblank.html',
              controller: 'WayBillBlankCtrl',
              label: 'БЛАНК'
          })
          .when(rootUrl + '/select_point/new_acceptance/blank', {
              templateUrl: staticPath + 'waybillblank.html',
              controller: 'WayBillBlankCtrl',
              label: 'БЛАНК'
          })
          .when(rootUrl + '/select_mail/new_acceptance/blank', {
              templateUrl: staticPath + 'waybillblank.html',
              controller: 'WayBillBlankCtrl',
              label: 'БЛАНК'
          })
