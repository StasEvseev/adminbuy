...
from applications.return_app.model import Return
...
class Mail(db.Model):
    ...
    # date2 = db.Column(db.DateTime(timezone=True))
    ...
    provider = db.relationship('Provider', backref=db.backref('mails', lazy='dynamic'))
    invoice_id = db.Column(db.Integer, db.ForeignKey('invoice.id'))
    invoice = db.relationship('Invoice', backref=db.backref('mails', lazy='dynamic'))
    order_id = db.Column(db.Integer, db.ForeignKey('order.id'))
    order = db.relationship('Order', backref=db.backref('mails_from_order', lazy='dynamic'))
    return_id = db.Column(db.Integer, db.ForeignKey('return.id'))
    return_item = db.relationship(Return, backref=db.backref('mails_from_return', lazy='dynamic'))

    def __init__(self, title, date, from_, to, text, files=None, is_handling=False):
        self.title = title
        self.date = date
        self.from_ = from_
        self.to = to
