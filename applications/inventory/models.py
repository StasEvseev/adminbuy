...
class Inventory(db.Model):
    ...
    #Дата ревизии
    datetimenew = db.Column(db.DateTime(timezone=True), default=dtmodule.datetime.now)
    location_id = db.Column(db.Integer, db.ForeignKey('point_sale.id'))
    location = db.relationship('PointSale', backref=db.backref('inventors', lazy='dynamic'))
    ...
    def __unicode__(self):
        return u"Инвентаризация №(%s)%s в %s" % (
            self.number, u" от %s" % self.datetimenew if self.datetimenew else u"", self.location)
...
class InventoryItems(db.Model):
    ...
    inventory = db.relationship(Inventory, backref=db.backref('items', lazy='dynamic'))

    # #Товар в системе
    good_id = db.Column(db.Integer, db.ForeignKey('good.id'))
    good = db.relationship('Good', backref=db.backref('inventarisations', uselist=False))
    #
