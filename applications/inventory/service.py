...
class InventoryService(BaseSQLAlchemyModelService):
    ...
    class InventoryServiceException(BaseSQLAlchemyModelService.ServiceException):
        pass
    ...
    def initial_inventory(cls, obj):
        ...
        try:
            ...
            if location_id:
                ...
                for item in items:
                    rev_item = cls.create_item(obj.id, item.good_id, item.count, item.count)
    ...
    def status(cls, inv, status):
        debug(u"Смена статуса `инвентаризации` %s с %s на %s" % (inv.id, inv.status, StatusType[status]))
        inv.status = status
        if status == VALIDATED:
            cls.sync_to_point(inv)

    @classmethod
    def create_item(cls, inventory_id, good_id, count_before, count_after=None):
        return InventoryItems(inventory_id=inventory_id, good_id=good_id, count_before=count_before,
        ...
        try:
            ...
            if location_id:
                for item in obj.items:
                    pointitem = PointSaleService.sync_good(location_id, item.good_id, item.count_after)
    ...
    def sync_from_json(cls, obj, json):
        ...
        if obj.status not in [VALIDATED, DRAFT]:
            ...
            obj.items.delete()
            debug(u"Старт: Заполнение новых записей в инвентаризацию (%s)" % obj.id)
            for item in json:
                if COUNT_AFTER_ATTR not in item:
                    ...
                    raise InventoryService.InventoryServiceException(
                        u"Товар %s без количества после инвентаризации, не может быть сохранен в (%s)." % (
                            good.full_name, obj))
                in_item = cls.create_item(
                    obj.id, item[GOOD_ATTR][GOOD_ID_ATTR],
                    item[COUNT_BEFORE_ATTR] if COUNT_BEFORE_ATTR in item and item[COUNT_BEFORE_ATTR] else 0,
                    item[COUNT_AFTER_ATTR])
                db.session.add(in_item)
            debug(u"Конец: Заполнение новых записей в инвентаризацию (%s)" % obj.id)
        else:
            debug(u"В статусе %s и %s позиции в инвентаризацию (%s) не сохраняются." % (VALIDATED, DRAFT, obj.id))