...
class Good(db.Model):
    ...
    def full_name_with_price(self):
        return self.full_name + \
                u" (" + \
               (u"розн. - " + unicode(float(self.price.price_retail)) + u", " if self.price and self.price.price_retail else u"") + \
               (u"опт. - " + unicode(float(self.price.price_gross)) if self.price and self.price.price_gross else u"") + \
               u")"