...
from .model import Price, PriceParish
...
PriceStub = namedtuple(
    'PriceStub', ['id', 'id_commodity', 'full_name', 'number_local',
                  'number_global', 'NDS', 'id_good', 'price_prev',
...
class PriceService(object):
    ...
    def price_retail(cls, price_post):
        return price_post * float(cls.RATE_RETAIL())

    @classmethod
    def price_gross(cls, price_post):
        return price_post * float(cls.RATE_GROSS())
    ...
    @classmethod
    def priceparish_to_commodity(cls, commodity_id):
        return PriceParish.query.filter(
            PriceParish.commodity_id == commodity_id
        )

    @classmethod
    def get_priceparish_strong(cls, commodity_id, number_local=None, number_global=None):
        """
